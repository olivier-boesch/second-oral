...
| **IDOR** | `fiche_salle`/`fiche_loge` (`/generate-doc-one`) : auth obligatoire ; canaux SSE soumis à autorisation par session |
| **Fiche candidat (login_key + QR)** | Servie par `/fiche-candidat/<id>.pdf`, générée à la demande dans un répertoire temporaire et jamais écrite dans `generated/` — admin ou le candidat lui-même uniquement (même règle que `show_credentials`). Avant : la génération était bien restreinte, mais le PDF persistait ensuite dans `generated/` sous `candidat_<NOM>.pdf` (nom devinable) et `/download` ne demandait qu'une session authentifiée quelconque — tout candidat, examinateur ou loge pouvait récupérer la fiche (donc le mot de passe et le token d'auto-connexion) de n'importe quel autre candidat. `liste_fiches_candidats` (le PDF groupé) écrit désormais ses fiches individuelles dans un répertoire temporaire, jamais dans `file_dir` |
| **`/download` — liste blanche** | Refus par défaut : seuls les motifs de `_DOWNLOAD_RULES` sont servis (papillons/`liste_candidats.pdf` → admin ; `liste_oraux`/`liste_salles`/`liste_loges`/`salle-*`/`loge-*` → personnel). Avant : cascade de préfixes terminée par un `else` permissif, où tout document non préfixé — dont `liste_candidats.pdf`, concaténation de toutes les fiches — retombait dans le cas large (accessible à tout utilisateur authentifié, pas seulement l'admin) |
| **Émargement (propriété de l'oral)** | `/sign` (GET et POST) et `/request-token` exigent que l'oral visé appartienne à l'examinateur connecté (`_emargement_autorise`, comparaison sur `identifiant` renvoyé par `SELECT_SIGNATURE_ORAL`) — l'admin seul émarge n'importe quel oral. `id_oral` étant choisi par le client et `UPDATE_SIGNATURE_ORAL` ne filtrant que sur `Oral.id`, sans ce contrôle un examinateur pouvait signer, ou effacer via `cancel=1`, l'émargement d'un collègue |
| **Jeton d'émargement** | MAC `hash_password(clé_utilisateur + id_oral)` en session, comparé via `hmac.compare_digest` et **lié à un `id_oral` précis** (`_token_emargement_valide`) : le token de `/request-token` étant une capacité au porteur (`/sign-other-device` l'accepte sans authentification), il ne peut jamais viser un autre oral que celui du jeton |
| **Échappement côté client** | Tout contenu dynamique assemblé en HTML puis injecté via `innerHTML` passe par un `esc()` explicite (`jour_j.html` : IP de session, nom de loge, identifiants). `encodeURIComponent` ne couvre que les `href`, et la CSP ne rattrape pas ce cas (`script-src-attr` autorise `'unsafe-inline'`) |
