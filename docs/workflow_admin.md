...
- **Réaffectation rapide des salles (`/gestion/reassignation-loges`)** : tableau matriciel salles × loges (une ligne par salle, une colonne par loge) — cliquer sur la case d'une loge y déplace immédiatement la salle correspondante (radio bouton, une case cochée par ligne = la loge actuelle), sans quitter la page ni repasser par le formulaire d'édition examinateur. Une colonne « + Ajouter » permet de créer une nouvelle loge à la volée (même mécanisme que l'option ➕ Nouvelle loge… ci-dessus). Chaque réaffectation notifie en direct (SSE) les vues salle/loge concernées, déjà ouvertes ou non. Accessible depuis la barre latérale admin, `/gestion/liste-examinateurs` et `/gestion/liste-loges`.
