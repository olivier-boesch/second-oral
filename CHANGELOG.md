...
- Vérifié manuellement via Chromium headless sur les 3 pages, avec les vraies feuilles de style du site (`main.css`/`theme.css` récupérées via le client de test) : rendu de la bulle, sélection par roue + confirmation `OK` → valeur du champ mise à jour, bouton `Vider` → champ vidé — pas de test automatisé (aucun harnais JS dans ce projet, uniquement Python)
