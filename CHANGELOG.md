...
**Fiche candidat — suppression de l'artefact PDF persistant (IDOR)**
- La génération de la fiche PDF d'un candidat (`login_key` en clair + QR d'auto-connexion valide 48h) était déjà restreinte à l'admin ou au candidat lui-même — mais le fichier produit persistait ensuite dans `generated/` sous `candidat_<NOM>.pdf`, un nom devinable. `/download` ne demandait alors qu'une session authentifiée quelconque, sans revérifier le propriétaire : tout candidat, examinateur ou surveillant de loge pouvait télécharger la fiche — donc le mot de passe et le token d'auto-connexion — de n'importe quel autre candidat.
- Nouvelle route `/fiche-candidat/<id>.pdf` (remplace le passage par `/generate-doc-one/fiche_candidat-<id>` puis `/download`) : génère le PDF dans un `TemporaryDirectory`, le relit, puis le sert directement dans la réponse. Le fichier n'existe jamais sur disque en dehors de la durée de la requête. `/generate-doc-one` ne traite plus `fiche_candidat` (404) ; `fiche_salle`/`fiche_loge` inchangés.
- `reports.liste_fiches_candidats()` (le PDF groupé, mêmes données sensibles par candidat) écrivait aussi ses fiches individuelles dans `file_dir` (= `generated/`) avant concaténation. Elles sont désormais écrites dans le répertoire temporaire de la fonction, jamais dans `file_dir` — seule `liste_candidats.pdf` (la concaténation) en ressort. Nommage indexé (`candidat_00000_...`) au passage : deux candidats homonymes produisaient auparavant le même nom de fichier temporaire et s'écrasaient l'un l'autre.
- Nouveau `reports._safe_filename_part()` : les noms de fichiers dérivés de données importées (nom de candidat/examinateur, salle) passent désormais par un filtrage anti path-traversal (`/`, `..`) avant d'entrer dans un chemin, appliqué à `fiche_candidat`, `salle_oraux` et `loge_oraux`.
- `/download` passe d'une cascade de préfixes terminée par un `else` permissif à une liste blanche (`_DOWNLOAD_RULES`) avec refus par défaut. La cascade permissive est précisément ce qui avait laissé passer `liste_candidats.pdf` — ne commençant par aucun préfixe connu (`papillons_`, `candidat_`), elle retombait dans le cas large et était donc accessible à tout utilisateur authentifié, examinateur et loge compris, pas seulement l'admin. Les `candidat_*.pdf` résiduels (générés par les versions antérieures) sont désormais explicitement refusés, y compris à l'admin.
- `app._purger_fiches_candidats_residuelles()` : supprime au premier passage de `_regenerer_fiches_candidats()` les `candidat_*.pdf` laissés dans `generated/` par les instances déjà déployées — ils ne sont plus servis mais restaient lisibles directement en cas de fuite de chemin par ailleurs.
...
- **Seul le retard évitable est pénalisé.** Chaque examinateur reçoit un cutoff personnalisé (`AlgoOne._cutoff_minutes_examinateur()`) repoussé jusqu'à sa fin minimale forcée : celle de son `AlgoOne._charge_plancher()`-ième créneau assignable, où le plancher vaut `floor(n_oraux_matière / n_examinateurs_matière)` (`floor` et non `ceil` — sous-estimer la charge forcée maintient le statu quo, la surestimer excuserait un retard évitable). Un examinateur dont la journée commence tard bénéficie du même report, les `CreneauInterdit` ne comptant pas dans le décompte. Si le plancher dépasse sa capacité, il est saturé par construction et sa pénalité tombe à zéro.
- Ce plancher est déduit des **seules données** (nombre d'oraux de la matière, nombre d'examinateurs), jamais de la solution en cours : une exemption conditionnée à une grille effectivement pleine inciterait le solveur à saturer l'examinateur qui finit le plus tard, puisque la saturation éteindrait sa pénalité — il resterait donc une constante du modèle, sans gradient exploitable.
- `AlgoCP._minutes_creneau()` et `_minutes_fin_creneau()` remontent sur **`AlgoOne`**, avec les deux nouvelles méthodes de plancher, pour que les deux moteurs partagent exactement le même cutoff personnalisé. Nouveau helper `AlgoOne._minutes_depuis_debut()`.
...
- Dépendance `pillow` relevée de `12.2.0` à `12.3.0` (`requirements.txt`) — corrige 5 vulnérabilités connues détectées par `pip-audit` (PYSEC-2026-2253 à 2257).
